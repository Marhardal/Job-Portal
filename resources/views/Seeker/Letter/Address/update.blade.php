    ...
    <div class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
                <form action="{{ URL::to('letter/address/'.$address->id) }}" method="post"  class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl md:max-w-2xl sm:mx-auto">
                    @method('patch')
                    @csrf
                    <div>
                        <div class="bg-white px-4 py-2 sm:p-3 sm:pb-2">
                            <div class="sm:flex sm:items-start">
                                <div class="text-center sm:mx-auto sm:mt-0 sm:text-left w-full">
                                    ...
                                    <x-form.text name="postal_code" placeholder="Enter your Postal Code" class="my-4" value="{{ $address->postal_code }}"/>
                                    <x-form.text name="addressing" placeholder="Who are you Addressing." class="my-4" value="{{ $address->addressing }}"/>
                                    <div class="mb-3">
                                        <div class="">
                                            ...
                                            <select name="address_type_id" id="type"
                                                ...
                                                @foreach ($type as $item)
                                                    <option value="{{ $item['id'] }}" @selected(old('address_type_id') == $item['id'] | $item['id']==$address->id )>
                                                        {{ ucwords($item['type']) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <div class="">
                                                ...
                                                <select name="city" id="city"
                                                    ...
                                                    @foreach ($district->data as $item)
                                                        <option value="{{ $item['name'] }}" @selected(old('city') == $item['name'] | $item['name']==$address->city)>
                                                            {{ ucwords($item['name']) }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="">
                                                ...
                                                <select name="country" id="country"
                                                    ...
                                                    @foreach ($countries->data as $item)
                                                        <option value="{{ $item['name'] }}" @selected(old('country') == $item['name'] | $item['name']==$address->country)>
                                                            {{ ucwords($item['name']) }}
                                                        </option>
                                                    @endforeach
                                                </select>
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                            <x-form.button value="Next" class="inline-flex w-full justify-center bg-green-600 hover:bg-green-500 sm:ml-3 sm:w-auto h-10 sm:mt-3">Update</x-form.button>
