...
<div {{ $attributes->merge(['class'=>'']) }}>
    <div class="relative">
        <textarea name="{{ $name }}" id="" cols="30" rows="7" class="peer w-full border-2 rounded-md py-2 border-gray-300 focus:border-blue-600 px-2 focus:outline-none transition-colors placeholder-transparent focus:top-3 resize-none" placeholder="{{ ucwords($placeholder) }}" id="{{ $name }}" >{{ old($name) }}</textarea>
